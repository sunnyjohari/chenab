...
void finish_with_errorss(MYSQL *con) {
...
int insert_to_esb_request ( char * sender_id, char * dest_id, char * message_type,      \
                   ...
                   char * status,char * status_details,char * received_on){
...
if (param_count != 10)/* alidate parameter count */
{
...
if (mysql_query(conn, "SELECT * FROM esb_request")) 
 ...
 strptime(received_on, "%Y-%m-%dT%H:%M:%S+%Z",&res);
